...
class MetricsCollector:
    ...
    def get_metric_series(
        ...
        if tags:
            snapshots = [
                s for s in snapshots if all(s.tags.get(k) == v for k, v in tags.items())
            ]
...
class DriftDetector:
    ...
    def detect_distribution_drift(
        self, reference_data: pd.Series, test_data: pd.Series, method: str = "ks"
    ...
    def detect_performance_degradation(
        self, recent_returns: pd.Series, baseline_sharpe: float, window: int = 63
    ) -> Tuple[bool, float, str]:
        """
        ...
        current_sharpe = (
            recent.mean() / recent.std() * np.sqrt(252) if recent.std() > 0 else 0
        )
        ...
        message = (
            f"Current Sharpe: {current_sharpe:.2f}, Baseline: {baseline_sharpe:.2f}"
        )
...
class PerformanceMonitor:
    ...
    def compare_to_baseline(
        self, current_sharpe: float, strategy_name: str
    ) -> Dict[str, Any]:
        """
        ...
        degradation_pct = (
            (current_sharpe - baseline_sharpe) / abs(baseline_sharpe)
