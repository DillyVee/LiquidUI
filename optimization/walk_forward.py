...
class WalkForwardAnalyzer:
    ...
    def run_walk_forward(
        ...
        **optimizer_kwargs
        ...
        print(f"   Date range: {df_finest['Datetime'].min().date()} to {df_finest['Datetime'].max().date()}")
    ...
    def _prepare_data(df_dict: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for tf, df in df_dict.items():
            ...
            if hasattr(df_copy['Datetime'].dtype, 'tz') and df_copy['Datetime'].dtype.tz is not None:
    ...
    def plot_walk_forward_results(results: WalkForwardResults, ticker: str = ""):
        ...
        ax2.plot(is_cumulative, color='#2979ff', linewidth=2, 
                label=f'IS: ${cumulative_is:.0f}')
        ax2.plot(oos_cumulative, color='#ff9800', linewidth=2, 
