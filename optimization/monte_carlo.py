...
class AdvancedMonteCarloAnalyzer:
    ...
    def calculate_advanced_metrics(
        ...
        all_returns_flat = all_returns_flat[
            ~(np.isnan(all_returns_flat) | np.isinf(all_returns_flat))
        ]
        ...
        # 7. TAIL RISK
        tail_ratio = (
            np.percentile(returns_pct, 95) / abs(np.percentile(returns_pct, 5))
            if np.percentile(returns_pct, 5) != 0
            else 0
        )
    ...
    def _calculate_var_metrics(
        returns_pct: np.ndarray, final_equities: np.ndarray, initial_equity: float
    ...
    def _calculate_path_dependency(
        simulations: List[np.ndarray], original_equity: float
    ) -> Tuple[float, float]:
        """
    ...
    @staticmethod
    def _bootstrap_ci(
        data: np.ndarray, confidence: float = 0.95, n_bootstrap: int = 1000
    ) -> Tuple[float, float]:
...
"""
    ...
    def plot_enhanced_distributions(
        ...
        ax1.axvline(
            ...
            linestyle="--",
            linewidth=2,
            label=f"Original: ${basic_results.original_equity:.0f}",
        )
        ax1.axvline(
            basic_results.mean_equity,
            color="#FFA500",
            linestyle="-",
            linewidth=2,
            label=f"Mean: ${basic_results.mean_equity:.0f}",
        )
        ax1.axvline(
            basic_results.percentile_5,
            color="#ff4444",
            linestyle=":",
            linewidth=2,
            label=f"5th %ile: ${basic_results.percentile_5:.0f}",
        )
        ax1.axvline(
            basic_results.percentile_95,
            color="#44ff44",
            linestyle=":",
            linewidth=2,
            label=f"95th %ile: ${basic_results.percentile_95:.0f}",
        )
        ...
        ax2.axvline(
            ...
            linestyle="-",
            linewidth=2,
            label=f"Mean: {advanced_metrics.max_drawdown_mean:.1f}%",
        )
        ax2.axvline(
            advanced_metrics.max_drawdown_95,
            color="#ff0000",
            linestyle="--",
            linewidth=2,
            label=f"95th: {advanced_metrics.max_drawdown_95:.1f}%",
        )
        ...
        ax3.axvline(
            ...
            linestyle="-",
            linewidth=2,
            label=f"Mean: {advanced_metrics.sharpe_mean:.2f}",
        )
        ...
        ax7.axvline(
            ...
            linestyle="-",
            linewidth=2,
            label=f"Mean: {advanced_metrics.win_rate_mean*100:.1f}%",
        )
        ...
        if calmar_ratios:
            ...
            ax8.axvline(
                ...
                linestyle="-",
                linewidth=2,
                label=f"Mean: {advanced_metrics.calmar_mean:.2f}",
            )
...
class MonteCarloSimulator:
    ...
    def simulate_trade_randomization(
        trades: List[Dict], n_simulations: int = 1000, initial_equity: float = 1000.0
    ) -> MonteCarloResults:
        """
    ...
    def generate_monte_carlo_report(
        results: MonteCarloResults, initial_equity: float = 1000.0
    ) -> str:
